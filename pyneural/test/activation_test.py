...
class TestDataManip(unittest.TestCase):
    ...
    def test_sigmoid(self):
        ...
        f1 = np.empty_like(f)
        ac.sigmoid(x, out=f1)
        assert np.all(np.equal(f, f1))
        g = ac.sigmoid_grad(f)
        g1 = np.empty_like(g)
        ac.sigmoid_grad(f, out=g1)
        assert np.all(np.equal(g, g1))
    ...
    def test_relu(self):
        ...
        assert np.all(np.equal(g, g1))
        assert f.shape == (3, ) and g.shape == (3, )
        assert np.all(np.equal(f, np.array(expected_f)))
        assert np.all(np.equal(g, np.array(expected_g)))
        ...
        assert np.all(np.equal(g, g1))
        assert f.shape == (2, 3) and g.shape == (2, 3)
        assert np.all(np.equal(f, np.array(expected_f1)))
        assert np.all(np.equal(g, np.array(expected_g1)))
